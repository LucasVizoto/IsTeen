...
import { listGames, Game } from '@/api/api';
import { Search, Calendar, Code2, Gamepad2, Loader2, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function GamesDashboard() {
  ...
  const [hasMore, setHasMore] = useState(true); // Controla se habilita o botão "Próximo"

  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState('');

  // Função de busca atualizada para aceitar página
  const fetchGames = useCallback(async (query: string, pageNumber: number) => {
    ...
    try {
      ...
      // Lógica: Se vieram menos de 20 itens, chegamos ao fim da lista.
  ...
  // 1. Efeito do Debounce (Busca)
  // Quando o usuário digita, resetamos para a página 1 e buscamos
  useEffect(() => {
  ...
  // 2. Função para mudar de página (Next/Prev)
  const handlePageChange = (newPage: number) => {
    if (newPage < 1) return;
    setPage(newPage);
    fetchGames(searchTerm, newPage);
    // Rola a tela para o topo suavemente ao trocar de página
                ...
                {games.map((game) => (
                  <div 
                    ...
                    className="
                      ...
                      rounded-2xl 
                      overflow-hidden 
                      hover:border-neon-pink hover:shadow-[0_0_20px_rgba(255,105,180,0.4)] hover:-translate-y-1
                  ...
                  >
                    ...
                    <div className="relative h-48 w-full overflow-hidden bg-black/50">
                      ...
                      <img 
                        ...
                        onError={(e) => {
                          ...
                          target.src = 'https://placehold.co/600x400/151525/FFF?text=No+Image';
                        }}
                      />
                    ...
                    <div className="p-5 flex flex-col flex-grow">
                      ...
                      <Link 
                        ...
                        target="_blank"
                        className="
                          mt-auto flex items-center justify-center gap-2 w-full py-2.5 
                          bg-white/5 border border-white/20 text-white font-semibold text-sm rounded-lg
            ...
            {games.length > 0 && (
              <div className="mt-12 flex items-center justify-center gap-6">
                ...
                <button
                  ...
                  className={`
                    flex items-center gap-2 px-6 py-3 rounded-full font-bold border-2 transition-all duration-300
                    ${page === 1 
                      ? 'border-white/10 text-white/30 cursor-not-allowed' 
                      : 'border-neon-cyan text-neon-cyan hover:bg-neon-cyan hover:text-black shadow-[0_0_15px_rgba(0,255,255,0.3)]'
                    }
                  `}
                >
                  <ChevronLeft size={20} />
                ...
                {/* Indicador de Página */}
                <div className="font-mono text-white/80 bg-black/40 px-4 py-2 rounded-lg border border-white/10">
                  Página <span className="text-neon-pink font-bold text-xl">{page}</span>
                </div>
